 ...
 *
 * Bottom-sheet with the full event details.
 * Shown when a user taps any event card.
 *
 * Props:
 *   event        : calendarEvent object (real or virtual)
 *   location     : location object | null
 *   memberContext: { clubId, memberId } | null  — the viewing member
 *   coachView    : boolean — show coach actions
 *   onClose      : () => void
 *   onEdit       : (event) => void  — coach/admin edit
 *   onCancel     : (event) => void  — coach/admin cancel
 */

import { useState, useEffect } from 'react';
import {
  X, MapPin, Clock, Users, Star, Trophy, Dumbbell,
  CheckCircle2, XCircle, AlertCircle, ChevronRight,
  Edit2, Trash2, ExternalLink, FileText,
} from 'lucide-react';
import {
  getEventColor, formatTs, formatDuration,
  durationFromEvent, isCheckInOpen, canSelfExcuse, EVENT_TYPES,
...
// ─── Main sheet ───────────────────────────────────────────────────────────────
export default function EventDetailSheet({ event, location, memberContext, coachView, onClose, onEdit, onCancel }) {
  ...
  // Load own attendance if member context available
  useEffect(() => {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      // If event is virtual, it gets materialised automatically by AttendanceFactory.upsert
      // (Firestore will create the path even if the parent doc doesn't exist yet)
      ...
      console.error('[EventDetailSheet] check-in error:', e);
  ...
  const handleExcuse = async (reason) => {
    ...
    try {
      ...
      console.error('[EventDetailSheet] excuse error:', e);
        ...
        <div style={{ padding: '0 16px 20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          {event.memberNotes && (
            <div style={{ backgroundColor: '#0f172a', borderRadius: '10px', border: '1px solid #334155', padding: '10px 12px' }}>
              <div style={{ fontSize: '11px', color: '#64748b', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: '5px' }}>
          ...
          {memberContext && attendanceCfg && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: attendanceCfg.color + '11', border: `1px solid ${attendanceCfg.color}33`, borderRadius: '10px', padding: '10px 12px' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '13px', fontWeight: '700', color: attendanceCfg.color }}>
                  {attendanceCfg.label}
                </div>
                {ownAttendance?.absentReason && (
                  <div style={{ fontSize: '11px', color: '#64748b', marginTop: '1px' }}>
                    Reden: {ownAttendance.absentReason}
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              {checkInOpen && ownAttendance?.status !== 'present' && (
                <button
                  ...
                  style={{ flex: 1, minWidth: '140px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '12px', backgroundColor: '#22c55e', border: 'none', borderRadius: '10px', color: 'white', fontWeight: '700', fontSize: '14px', cursor: 'pointer', opacity: actionLoading ? 0.65 : 1, fontFamily: 'inherit' }}
                >
              ...
              {canExcuse && ownAttendance?.status !== 'excused' && (
                <button
                  ...
                  disabled={actionLoading}
                  style={{ flex: 1, minWidth: '140px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '12px', backgroundColor: 'transparent', border: '1px solid #f59e0b44', borderRadius: '10px', color: '#f59e0b', fontWeight: '600', fontSize: '14px', cursor: 'pointer', opacity: actionLoading ? 0.65 : 1, fontFamily: 'inherit' }}
                >
          ...
          {coachView && (
            <div style={{ borderTop: '1px solid #334155', paddingTop: '14px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              <button onClick={() => onEdit?.(event)} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '9px 14px', backgroundColor: 'transparent', border: '1px solid #334155', borderRadius: '8px', color: '#94a3b8', fontWeight: '600', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}>
                <Edit2 size={13} /> Bewerken
              </button>
              {!isCancelled && (
                <button onClick={() => onCancel?.(event)} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '9px 14px', backgroundColor: 'transparent', border: '1px solid #ef444433', borderRadius: '8px', color: '#ef4444', fontWeight: '600', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}>
