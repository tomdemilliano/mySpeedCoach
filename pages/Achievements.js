...
import { BadgeFactory, UserFactory } from '../constants/dbSchema';
import {
  Trophy, Target, Medal, Award, Check, ChevronDown, ChevronUp,
  Zap, TrendingUp, Plus, Trash2, ArrowLeft, Star, Hash
} from 'lucide-react';
...
function TabBtn({ active, onClick, icon: Icon, label, color, count }) {
  return (
    ...
    >
      ...
      <span style={{ fontSize: '12px', fontWeight: active ? '700' : '500', color: active ? color : '#64748b' }}>
        {label}
      </span>
      {count != null && (
        <span style={{
...
function BadgeItem({ badge, earned, earnedDate, awardedByName, note }) {
  ...
  return (
    <div
      style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', cursor: 'pointer' }}
    ...
    >
      <div style={{
        ...
        {badge.badgeImageUrl || badge.imageUrl ? (
          <img src={badge.badgeImageUrl || badge.imageUrl} alt={badge.badgeName || badge.name}
            style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '50%' }} />
        ) : (
          <span>{badge.badgeEmoji || badge.emoji || '🏅'}</span>
        )}
      </div>
      <div style={{ fontSize: '10px', color: earned ? '#94a3b8' : '#334155', textAlign: 'center', maxWidth: '64px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}>
        <div style={css.statCard}>
          <div style={{ fontSize: '28px', fontWeight: '900', color: '#f59e0b', lineHeight: 1 }}>{earnedBadges.length}</div>
          <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>Verdiend</div>
        </div>
        <div style={css.statCard}>
          <div style={{ fontSize: '28px', fontWeight: '900', color: '#475569', lineHeight: 1 }}>{allBadges.length - earnedBadges.length}</div>
          <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>Te verdienen</div>
        </div>
      </div>
      ...
      {earnedBadges.length > 0 && (
        <div style={{ marginTop: '24px', borderTop: '1px solid #1e293b', paddingTop: '16px' }}>
          <div style={{ fontSize: '11px', color: '#64748b', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '10px' }}>
            Recent verdiend
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {earnedBadges.slice(0, 5).map(b => (
              <div key={b.id} style={{
                ...
                backgroundColor: '#1e293b', borderRadius: '10px', padding: '10px 14px',
...
function RecordsTab({ uid }) {
  ...
  useEffect(() => {
    ...
    DISCIPLINES.forEach(d => SESSION_TYPES.forEach(st => {
      const u = UserFactory.subscribeToRecords(uid, d, st, (rec) => {
        if (rec) {
          setRecords(prev => {
  ...
  return (
    <div>
      ...
      {bestOverall && (
        <div style={{
          backgroundColor: '#1e293b', borderRadius: '14px', padding: '20px',
          border: '1px solid #facc1533', marginBottom: '20px',
          display: 'flex', alignItems: 'center', gap: '16px',
        }}>
          <div style={{
            width: '56px', height: '56px', borderRadius: '14px',
            backgroundColor: '#facc1522', border: '1px solid #facc1544',
          ...
          <div>
            <div style={{ fontSize: '11px', color: '#64748b', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              Beste persoonlijk record
            </div>
            <div style={{ fontSize: '32px', fontWeight: '900', color: '#facc15', lineHeight: 1 }}>
              {bestOverall.score}
              <span style={{ fontSize: '14px', color: '#94a3b8', fontWeight: '400', marginLeft: '6px' }}>stappen</span>
            </div>
            <div style={{ fontSize: '12px', color: '#64748b', marginTop: '2px' }}>
              {DISC_LABELS[bestOverall.discipline]} · {bestOverall.sessionType}
            </div>
          </div>
        </div>
      )}
      ...
      {records.length === 0 ? (
        <div style={css.emptyState}>
          <Trophy size={36} color="#334155" />
          <p style={{ color: '#475569', fontSize: '14px', margin: '12px 0 0' }}>Nog geen records</p>
          <span style={{ color: '#334155', fontSize: '12px' }}>Records worden automatisch bijgehouden na sessies.</span>
        </div>
      ) : (
        <div>
          {DISCIPLINES.map(d => {
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {discRecords.map(rec => (
                    <div key={rec.id} style={{
                      backgroundColor: '#1e293b', borderRadius: '10px', padding: '12px 14px',
                      border: '1px solid #334155',
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <span style={{
                          ...
                          backgroundColor: rec.sessionType === 'Wedstrijd' ? '#ef444422' : '#3b82f622',
                          color: rec.sessionType === 'Wedstrijd' ? '#ef4444' : '#60a5fa',
                          border: `1px solid ${rec.sessionType === 'Wedstrijd' ? '#ef444440' : '#3b82f640'}`,
                        }}>
                          {rec.sessionType}
                        </span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
                        <span style={{ fontSize: '22px', fontWeight: '900', color: '#facc15' }}>{rec.score}</span>
                        <span style={{ fontSize: '11px', color: '#64748b' }}>stappen</span>
                      </div>
                    </div>
                  ))}
                </div>
...
function GoalsTab({ uid }) {
  ...
  useEffect(() => {
    if (!uid) return;
    const unsub = UserFactory.getGoals(uid, setGoals);
    const unsubs2 = [];
    DISCIPLINES.forEach(d => SESSION_TYPES.forEach(st => {
      const u = UserFactory.subscribeToRecords(uid, d, st, (rec) => {
        if (rec) {
  ...
  const handleAdd = async () => {
    if (!form.targetScore) return;
    const { db: firestoreDb } = await import('../firebaseConfig');
    const { addDoc, collection } = await import('firebase/firestore');
    await addDoc(collection(firestoreDb, `users/${uid}/goals`), {
  ...
  const handleDelete = async (goalId) => {
    if (!window.confirm('Doel verwijderen?')) return;
    const { db: firestoreDb } = await import('../firebaseConfig');
    const { deleteDoc, doc } = await import('firebase/firestore');
    await deleteDoc(doc(firestoreDb, `users/${uid}/goals`, goalId));
  };

  const getBestRecord = (discipline) => {
    return records.filter(r => r.discipline === discipline)
      .reduce((best, r) => r.score > (best?.score || 0) ? r : best, null);
  };
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <div>
          <div style={{ fontSize: '13px', color: '#64748b' }}>
            {activeGoals.length} actief · {achievedGoals.length} bereikt
          </div>
        </div>
        <button
          ...
          style={{
            ...
            padding: '8px 14px', backgroundColor: '#22c55e22',
      ...
      {goals.length === 0 ? (
        <div style={css.emptyState}>
          ...
          <p style={{ color: '#475569', fontSize: '14px', margin: '12px 0 0' }}>Geen doelen</p>
          <span style={{ color: '#334155', fontSize: '12px' }}>Stel doelen om je voortgang bij te houden.</span>
        </div>
      ) : (
        <div>
                  ...
                  return (
                    <div key={g.id} style={{ backgroundColor: '#1e293b', borderRadius: '12px', padding: '14px', border: '1px solid #334155' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                        <div>
                          <div style={{ fontWeight: '700', fontSize: '15px', color: '#f1f5f9' }}>
                            {DISC_LABELS[g.discipline]} — {g.targetScore} stappen
                          </div>
                          {g.targetDate && (
                            <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>
                              Deadline: {new Date(g.targetDate?.seconds ? g.targetDate.seconds * 1000 : g.targetDate).toLocaleDateString('nl-BE')}
                            </div>
                          )}
                        </div>
                        <button
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <span style={{ fontSize: '12px', fontWeight: '700', color: progress >= 100 ? '#22c55e' : '#64748b', minWidth: '36px', textAlign: 'right' }}>
                          {progress}%
                        </span>
                      </div>
                      {best && (
                        <div style={{ fontSize: '11px', color: '#475569', marginTop: '6px' }}>
                          Huidig beste: <strong style={{ color: '#94a3b8' }}>{best.score}</strong> stappen
                        </div>
                      )}
                    </div>
                  );
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {achievedGoals.map(g => (
                  <div key={g.id} style={{
                    ...
                    <div style={{ width: '28px', height: '28px', borderRadius: '8px', backgroundColor: '#22c55e22', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {g.achievedAt?.seconds && (
                        <div style={{ fontSize: '11px', color: '#475569', marginTop: '1px' }}>
                          Bereikt op {new Date(g.achievedAt.seconds * 1000).toLocaleDateString('nl-BE')}
                        </div>
                      )}
                    </div>
                    <button onClick={() => handleDelete(g.id)} style={{ background: 'none', border: 'none', color: '#334155', cursor: 'pointer', padding: '4px' }}>
                      <Trash2 size={14} />
                    </button>
                  </div>
                ))}
          ...
          <div style={css.modal}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ margin: 0, color: '#f1f5f9', fontSize: '16px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <Target size={18} color="#22c55e" /> Doel toevoegen
              </h3>
            ...
            <input style={css.input} type="number" placeholder="bijv. 150"
            ...
            <button
              onClick={handleAdd}
              style={{ width: '100%', padding: '13px', backgroundColor: '#22c55e', border: 'none', borderRadius: '10px', color: 'white', fontWeight: '700', fontSize: '15px', cursor: 'pointer', marginTop: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
            >
  ...
  if (!uid) {
    return (
      <div style={{ backgroundColor: '#0f172a', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '16px', color: 'white', fontFamily: 'system-ui, sans-serif' }}>
        ...
        <p style={{ color: '#64748b', fontSize: '14px' }}>Log in om je prestaties te bekijken.</p>
  ...
  return (
    <div style={{ backgroundColor: '#0f172a', minHeight: '100vh', color: 'white', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '20px 16px 40px' }}>
        {activeTab === 'badges' && <BadgesTab uid={uid} />}
        {activeTab === 'records' && <RecordsTab uid={uid} />}
        {activeTab === 'goals' && <GoalsTab uid={uid} />}
      </div>
    </div>
  );
}
